...
def persisted_user(users: DjangoUserRepository) -> User:
    user = User.register(email=Email("repo@example.com"), display_name="Repo", password_hash="hash")
    users.add(user)
    return user
...
class TestUserRepository:
    def test_round_trip(self, users: DjangoUserRepository) -> None:
        user = User.register(email=Email("a@b.com"), display_name="X", password_hash="hash")
        users.add(user)
...
class TestRefreshTokenRepository:
    ...
    def test_invalidate_family_marks_all_active(
        ...
        for n in range(3):
            refresh_tokens.add(
                RefreshToken.issue(user_id=persisted_user.id, token=f"t{n}", family_id=family)
            )
...
class TestSingleUseTokens:
    def test_consume_is_atomic(
        ...
        tok = SingleUseToken.for_email_verification(user_id=persisted_user.id, token="single")
