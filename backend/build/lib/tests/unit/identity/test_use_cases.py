...
class TestRegister:
    ...
    def test_duplicate_email_raises(self, services: IdentityServices) -> None:
        register_user(
            services, email="a@b.com", display_name="X", password="CorrectHorseBattery!7"
        )
...
class TestPasswordReset:
    ...
    def test_round_trip_changes_password_and_invalidates_refresh(
        ...
        reset_token = next(
            tok for kind, _, tok in services.email_sender.sent if kind == "reset"  # type: ignore[attr-defined]
        )
        confirm_password_reset(
            services, token=reset_token, new_password="NewHorseBattery!42"
        )
        with pytest.raises(InvalidCredentials):
