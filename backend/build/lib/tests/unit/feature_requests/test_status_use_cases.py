...
class _StubRepo:
    ...
    def transition_status(self, *, request_id, expected_from, to_status, changed_by_user_id, reason, duplicate_of_id):
        fr = self.requests[request_id]
        updated = FeatureRequest(
            id=fr.id, title=fr.title, description=fr.description,
            author_id=fr.author_id, status=to_status,
            vote_count=fr.vote_count, duplicate_of_id=duplicate_of_id,
...
def _seed_fr(repo: _StubRepo, *, status=FeatureRequestStatus.OPEN) -> FeatureRequest:
    fr = FeatureRequest(
        id=uuid4(), title=Title("test"), description=Description(""),
        author_id=uuid4(), status=status, vote_count=1,
    )
...
class TestTransitionStatusUseCase:
    ...
    def test_non_duplicate_transition_clears_duplicate_of_id(self) -> None:
        ...
        _, expected_from, _, dup_id = (
            repo.transitions[-1][0],
            repo.transitions[-1][1],
            repo.transitions[-1][2],
            None,  # stub doesn't capture duplicate_of_id, but use case must pass None
        )
        # The stub call captured (request_id, expected_from, to_status); confirm it was reached
