...
def moderator():
    ...
    user = User.register(
        email=Email("mod@example.com"), display_name="Mod", password_hash="h"
    )
    users.add(user)
    return user
...
def _submit(repo: DjangoFeatureRequestRepository, *, author_id, title: str) -> FeatureRequest:
    fr = FeatureRequest.submit(
        title=Title(title), description=Description(""), author_id=author_id
    )
...
class TestTransitionStatus:
    ...
    def test_status_conflict_raised_when_expected_from_is_stale(self, author, moderator, repo) -> None:
