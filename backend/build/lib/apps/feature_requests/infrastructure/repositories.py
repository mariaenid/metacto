...
_HOT_SQL = (
    "(vote_count - 1) / POWER("
    "EXTRACT(EPOCH FROM (NOW() - created_at)) / 86400 + 2, 1.4)"
)
...
class DjangoFeatureRequestRepository:
    def submit_with_author_vote(self, request: FeatureRequest) -> None:
        ...
        with transaction.atomic():
            ...
            VoteRecord.objects.create(
                feature_request_id=request.id, user_id=request.author_id
            )
    ...
    def list(self, *, sort: SortOption, limit: int, offset: int) -> ListPage:
        ...
        if sort is SortOption.TOP:
            ...
            queryset = queryset.annotate(_hot=RawSQL(_HOT_SQL, [])).order_by(
                "-_hot", "-created_at"
            )
