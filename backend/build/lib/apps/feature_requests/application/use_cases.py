...
def get_feature_request(
    services: FeatureRequestServices, *, request_id: UUID
) -> FeatureRequest:
...
def retract_vote(
    ...
    *,
    ...
    cached = _load_cached(
        services, idempotency_key, user_id, "retract", feature_request_id
    )
    if cached is not None:
        return cached
    if services.requests.get_by_id(feature_request_id) is None:
        raise FeatureRequestNotFound(str(feature_request_id))
    result = services.votes.retract(
        feature_request_id=feature_request_id, user_id=user_id
    )
    _store_cached(
        services, idempotency_key, user_id, "retract", feature_request_id, result
    )
    return result
...
def _load_cached(
    ...
    if not idempotency_key:
        return None
    return services.idempotency.get(
        _idem_key(user_id, action, request_id, idempotency_key)
    )
...
def _store_cached(
    ...
    if not idempotency_key:
        return
    services.idempotency.put(
        _idem_key(user_id, action, request_id, idempotency_key), result
    )
