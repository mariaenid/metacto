...
class DjangoRefreshTokenRepository:
    ...
    def invalidate_family(self, family_id: UUID) -> None:
        RefreshTokenRecord.objects.filter(family_id=family_id, used_at__isnull=True).update(
            used_at=datetime.now(UTC)
        )

    def invalidate_all_for_user(self, user_id: UUID) -> None:
        RefreshTokenRecord.objects.filter(user_id=user_id, used_at__isnull=True).update(
            used_at=datetime.now(UTC)
        )
